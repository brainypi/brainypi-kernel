...
struct ov5647_state {
	...
	/* Rockchip specific variables added */
	u32 module_index;
...
{
	struct ov5647_state *sensor = to_state(sd);
	long ret = 0;
			...
			const struct i2c_device_id *id)
{
	...
	dev_info(dev, "OV5647 Rockchip specific driver");
	...
	if (IS_ENABLED(CONFIG_OF) && np) {
		...
		/* Parse Devicetree for Rockchip specific camera module information */
	...
	sensor->enable_gpio = devm_gpiod_get(dev, "enable", GPIOD_OUT_LOW);
	...
	delay_us = DIV_ROUND_UP(8192, xclk_freq / 1000 / 1000);
	...
	if (ret){
		dev_info(dev, "Failed to init controls\n");
		return ret;
	}
	...
	if (ret < 0) {
		dev_info(dev, "Failed to init media_pad\n");
		goto mutex_remove;
	}

	ret = ov5647_detect(sd);
	if (ret < 0){
		dev_info(dev, "Failed to detect ov5647\n");
		goto error;
	}
	...
	if (strcmp(sensor->module_facing, "back") == 0)
		snprintf(sd->name, sizeof(sd->name), 
				"m%02d_%s_%s %s", 
				sensor->module_index, "b", SENSOR_NAME, dev_name(sd->dev));
	else
		snprintf(sd->name, sizeof(sd->name),
				"m%02d_%s_%s %s", 
				sensor->module_index, "f", SENSOR_NAME, dev_name(sd->dev));

	dev_info(dev, "OV5647 sensor name: %s\n", sd->name);
